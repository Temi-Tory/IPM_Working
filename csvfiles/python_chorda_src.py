...
def complete_to_chordal_graph(G):
    ...
    """
    H = G.copy()
    alpha = {node: 0 for node in H} #dictionary for every node where key is niode and value is -0
    if nx.is_chordal(H):
        return H, alpha
    chords = set()
    weight = {node: 0 for node in H.nodes()}
    unnumbered_nodes = list(H.nodes())
    for i in range(len(H.nodes()), 0, -1):  # for loop starting from highest number 
        # get the node in unnumbered_nodes with the maximum weight
        z = max(unnumbered_nodes, key=lambda node: weight[node]) #find node with maximum value currently in the graph h 
        unnumbered_nodes.remove(z) #remove Z
        alpha[z] = i  #update  value of key Z to i
        update_nodes = [] #empty list 
        for y in unnumbered_nodes:  #check the remaining nodes 
            if G.has_edge(y, z):    #if there is edge between every other node and z
                update_nodes.append(y)   # if edge exsist, add it to nupdate node lkist 
            else:
                # y_weight will be bigger than node weights between y and z
                y_weight = weight[y]   #if no egde, create a variable called y_weight 
                lower_nodes = [
                    node for node in unnumbered_nodes if weight[node] < y_weight
                ]    # find all nodes that have lower weight than Y node
