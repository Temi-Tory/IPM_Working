...
fork--join subgraph where paths in the network fan out from a common node and fan back in at a node further downstream. The framework's decomposition is partition-forward, that is, each identified diamond is returned as a localised graph object of the same form as the full network, and can be handed to any analysis module in the framework on its own. Secondarily, a diamond can serve as a reductive unit, since a solved diamond can stand in for its subgraph as a single equivalent component.


\section{A Generic Decomposition Module}
\label{sec:dd-genericity}

The network decomposition module identifies and returns every instance of a subgraph pattern discovered in the input DAG, and what counts as the pattern is decided by three discriminator functions. One discriminates the nodes eligible to take part in a pattern at all. One decides what counts as the structure a pattern forms around. One decides when two paths into a node belong to the same pattern. The identification algorithm does the same work whatever the discriminators say.

A discriminator can be defined on any property in the unified graph object, or on any set of network characteristics mapped to a property in the unified graph. Each discriminator can be overridden or modified to fit the required definition. A user can adjust an existing discriminator (e.g. changing the threshold to 3 or more paths), extract a different characteristic of the network altogether (e.g. fan-in-then-fan-out instead of fan-out-then-fan-in), or discriminate on a characteristic that is not already a graph object property, mapped onto one during input processing (e.g. a component type supplied with the raw network, so that only one class of component can anchor a pattern).

In the Julia implementation of the framework,  the eligibility discriminator is exposed directly as a runtime override, and the other two are compact internal functions a replacement definition would swap.
...
\label{sec:dd-isdet}
The eligibility criterion is the one discriminator not drawn from the diamond's definition, and \texttt{is\_det} in the implementation is the only place identification reads declared values. Since IPF's decomposition module was built with probabilistic propagation in mind, the eligibility criterion excludes nodes whose prior probability is deterministic. That covers exactly two cases: any inactive node (i.e.\ unreachable with prior = $0$) or a perfect source node (i.e.\ always active with prior = $1$). The conjunction in the second case matters because a non-source node with prior = $1$ may still be probabilistically uncertain depending on its incoming edges. Therefore excluding every node valued $0$ or $1$ would incorrectly disable every diamond in a probabilistic DAG where all node priors happen to be $1$. For interval-valued and p-box-valued networks the two cases apply only to degenerate values, an interval or p-box collapsed exactly onto $0$ or $1$.
...
Given this, a diamond is defined formally.
...
IPF returns two views of the network's diamond composition.
...
\textbf{The maximal diamond} is the network-level view of a diamond join. It is the full subgraph the join's reconvergence forms, comprising the diamond of every group of the join's parents that forms one, spanning the join, the groups, and everything upstream of them, together with the edges of the join's non-influencing set. Every diamond join carries exactly one maximal diamond, while a fork can participate in several, and every diamond found at deeper levels inside it is a \textbf{sub-diamond}, its edge list contained in the enclosing diamond's.

A unique diamond with no inner sub-diamond is itself an induced diamond, the smallest a diamond can be. Since diamonds nest and overlap, the unique diamonds are exactly the distinct maximal diamonds together with their inner sub-diamonds.

\section{The Identification Algorithm}
\label{sec:dd-algorithm}

\texttt{identify\_diamonds} is a recursive set-membership algorithm over the closures of the unified graph object. For every join node of $G$ it identifies the join's maximal diamond, pairing the join with the earliest covering forks in its parents' shared ancestry, and it recurses inside each diamond it forms for the sub-diamonds. Each call works on a join $v$, a \emph{fixed context} $E \subseteq V$ of nodes already fixed by enclosing diamonds, and the parents $P$ of $v$ in view. The recursion starts from $E = \varnothing$, and Algorithm~\ref{alg:new-identify} shows the skeleton.
...
Disjointness is a statement about the influencing sets of the diamond groups: each group decomposes separately. Whether separately computed values recombine correctly at the join is a different claim, which Chapter~\ref{ch:probability-toolkit} makes and proves in its own terms. Parents in singleton groups fall into the join's non-influencing set.
...
\]
Write $V(\ED)$ for the set of its endpoints. The first two clauses keep the edge list on the relevant set $R$. The third removes every edge into a fixed node, whose upstream structure is already settled, while its outgoing edges stay and act as local sources of the diamond. The fourth admits an edge into the diamond join only from the group, closing a failure mode the first three cannot see.
...
Consider a nested level at which an enclosing diamond has already fixed a fork $r$, so $r \in E$, and suppose $r$ is both an ancestor of some group member $p$ and a direct parent of the join $v$. Partitioning handles $r$ correctly, since fixing removed $r$ from every influencing set, and in the minimal case of $r$ a source with no free ancestry its influencing set is empty, landing it in a singleton group whose contribution to $v$ enters separately.

The span $R$, built from the raw closure $\Anc(p)$, contains $r$ no matter what the context says, so partitioning and span silently diverge wherever $E \neq \varnothing$. The edge $(r,v)$ then has both endpoints in $R$ and a head outside $B$, so the first three clauses admit it, and a diamond meant to capture $v$ as reached through $G$ alone quietly gains a second unrelated entry into its own join. The fourth clause blocks exactly this edge. Figure~\ref{fig:isolation} shows the minimal instance.
...
The algorithm builds each unique diamond as a unified graph object of its own. The unique diamond object carries the diamond's characteristics together with its subgraph in the form of the unified graph object, so a diamond can be treated as a standalone DAG network for any other type of analysis through the framework, as Table~\ref{tab:dd-selfsimilar} shows.
...
Invoking the decomposition module on the unified graph object from the input module extends the object with the field \texttt{unique\_subgraphs}, holding the network's unique subgraphs. For this framework, the propagation algorithm of Chapter~\ref{ch:probability-toolkit} reduces solved diamonds to supernodes.
...
The nine-edge network of Figure~\ref{fig:factorised} has forks 1 and 2 and a single join at node 7, which source 8 also feeds directly. The parents split into two correlated groups, $\{3,4\}$ through fork 1 and $\{5,6\}$ through fork 2, with parent 8 related to neither. Two constituent diamonds arrive at join 7, one anchored on each fork, and node 8 rides along in the non-influencing set. Together they are the join's maximal diamond.
...
\textbf{Join 6, top level.} Join 6 is also a join of the network itself, so the top-level pass visits it in its own right. With $E = \varnothing$, forks 1 and 3 both cover parents 4 and 5, and the earliest covering fork rule fixes fork 1 rather than fork 3, which a reader tracing by eye might expect. The result is a second maximal diamond $D_2$ spanning $\{(1,3),(3,4),(3,5),(4,6),(5,6)\}$ with $C = \{1\}$, and the recursion inside it, now in context $\{1\}$, again produces exactly $D_3$.
...
The trace also confirms the edge-isolation case of Section~\ref{sec:dd-edges} end to end. In the network of Figure~\ref{fig:isolation}, the nested diamond at join 5 under $E=\{1\}$ is emitted with $C = \{2\}\cup(\{1\}\cap V(\ED)) = \{1,2\}$, node 1 staying among the edge list's endpoints through its surviving edge $(1,3)$, which is the context-aware identity visible in the output. Node 1 is recorded in the nested diamond's non-influencing set, and the edge $(1,5)$ appears nowhere in its edge list. At the top level node 1 is a member of the group itself, so the maximal diamond at join 5 carries $(1,5)$ legitimately.


\section{Summary}
\label{sec:dd-summary}

The module extends the graph object of Chapter~\ref{ch:input-module} with \texttt{unique\_subgraphs}, storing every diamond found at any level exactly once, each entry a graph object of the same form as the network it came from, with the maximal diamonds flagged. The decomposition terminates within $|V|$ nested steps and is canonical for a given input (Theorems~\ref{thm:termination} and~\ref{thm:determinism}). It groups a join's parents exactly by shared unfixed ancestry and produces a diamond wherever unsettled reconvergence exists and only there (Theorems~\ref{thm:grouping}--\ref{thm:completeness}). Nothing outside a diamond's span enters its edge list (Lemma~\ref{lem:isolation}), and the context-aware identity keeps apart the diamonds that must be kept apart (Proposition~\ref{prop:identity}).
