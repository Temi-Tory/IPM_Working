...
networks. Section~\ref{sec:dd-discussion} gives the module's limitations and the extension
...
Most graph-based analysis methods decompose. For large networks with complicated
connectivity they must \cite{perez2018sixty}. Broadly, decomposition breaks a network into substructures that
...
There is no universal unit of decomposition. Each method defines its own. In the series--parallel reduction reviewed in
...
\end{itemize}
The identification algorithm is the same whatever the discriminators say.
...
The eligibility criterion is the one discriminator not drawn from the diamond's definition. In the implementation, \texttt{is\_det} is the only place where identification reads declared values. Since the module was built with probabilistic propagation in mind, the default criterion excludes nodes whose prior probability is deterministic. There are two such cases: an inactive node, with prior $0$, and a perfect source, with prior $1$. The second case requires both conditions. A non-source node with prior $1$ may still be uncertain, because its incoming edges may fail, so excluding every node valued $0$ or $1$ would wrongly disable every diamond in a network where all priors happen to be $1$. For interval-valued and p-box-valued networks the two cases apply only to degenerate values, an interval or p-box collapsed exactly onto $0$ or $1$.
...
\]
with $\Anc(p)$ self-inclusive so that $p \in \infl(p,E)$ whenever $p \notin E$. Two parents $p, q$ of a join are \emph{structurally correlated under $E$} when $\infl(p,E) \cap \infl(q,E) \neq \varnothing$, that is, when some node upstream of both is not held fixed. Extending the context with a node removes it from every influencing set, as fixing a component does in factoring.
...
Disjointness here is a statement about the influencing sets of the diamond groups, namely that each group decomposes separately. Whether separately computed values recombine correctly at the join is a different claim, which Chapter~\ref{ch:probability-toolkit} makes and proves for the probabilistic interpretation. Parents in singleton groups fall into the join's non-influencing set.
...
Consider a nested level at which an enclosing diamond has already fixed a fork $r$, so $r \in E$, and suppose $r$ is both an ancestor of some group member $p$ and a direct parent of the join $v$. Partitioning places $r$ by its remaining free ancestry, since fixing removed $r$ from every influencing set. In the minimal case of $r$ a source with no free ancestry its influencing set is empty, and $r$ lands in a singleton group whose contribution to $v$ enters separately.
...
An edge list alone cannot place a diamond in the nesting hierarchy.
...
\section{Discussion}
\label{sec:dd-discussion}

Decomposition runs on a directed acyclic input. A network carrying cycles is decomposed only
after the orientation of Chapter~\ref{ch:system-model} has broken them, and the general
cyclic case is outside the module's scope.

Of the three discriminators, only eligibility is configurable at run time. An analysis that
needs a different account of which nodes may take part supplies its own test through the
override of Section~\ref{sec:dd-genericity}. The anchoring and membership discriminators are
internal functions of the implementation. Changing either is a change to the source, and
neither has a configuration route.

The diamond is the only pattern for which a set of discriminators has been defined. The
module's genericity over patterns rests on the separation between the identification
algorithm and the three discriminators, stated in Section~\ref{sec:dd-genericity}. No second
pattern has been defined here to exercise it.

A further analysis could define its own three discriminators and obtain every instance of a
different subgraph pattern from the same algorithm, each instance returned as a graph object
of the form given in Section~\ref{sec:dd-representation}. Chapter~\ref{ch:conclusions} lists
this with the framework's other extensions.
