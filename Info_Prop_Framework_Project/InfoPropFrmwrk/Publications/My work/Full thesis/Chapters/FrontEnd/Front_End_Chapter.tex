...
package, and a package is used by writing code. The engineer who holds the system model, a planner at a water utility or a decommissioning
programme, does not write code. The tools that do reach such a person are either cloud
...
This chapter presents the interface that closes that gap. It is a browser application that
...
tabs. For the reliability toolkit (Figure~\ref{fig:fe-reliability}) these are Belief, the
...
every summary tile names what it summarises. The flow and schedule pages have the same
shape, with their own tabs: configuration, summary, bottlenecks and visualisation for flow
...
any analysis has produced on the network, a critical path, a set of edges in every minimum
cut, a conditioning set, is listed, and any one of them can be drawn on the network with a
second overlaid, so that a schedule's critical path and a flow's binding cut can be seen on
the same nodes. The page computes nothing; it draws what the analyses already found.
...
interface. Where a table is sorted, the sort key is a field the toolkit returned. The
reason is that a number computed in the interface would be a number the thesis has not
validated.

Every result carries its method. The schedule toolkit returns interval margins from one of
three methods, and the reliability toolkit returns beliefs from an exact or a
decomposition-only call; in each case the response names the method, and the interface
shows the tag beside the result. A conservative enclosure and an exact range are therefore
never displayed the same way. The reason is Chapter~\ref{ch:critical-path}: the two differ
in what they guarantee, and a reader who cannot tell them apart cannot use either.

One analysis runs at a time. The Compare tab runs unrun scenarios in sequence, and no view
issues two requests to the server together. The reason is Chapter~\ref{ch:julia-package}:
the propagation is single-threaded and its cache is not locked, so a second concurrent
request would either wait or, at worst, corrupt a shared store.

One rule the interface does not yet enforce is recorded here because it was found in use.
...
access; a component in the presentational library that needs a scenario's shape therefore
...
been used independently by an engineer who does not program. The evaluation that can be
offered is therefore a task-based walkthrough, and it is labelled as such: it shows that
the tasks a user would set are reachable without code, and it does not show how a user
found them.
...
Chapter~\ref{ch:integrated-case-study}. What the walkthrough cannot show is the time a new
user takes, where the labels are unclear, and which of the four rules of
...
which the user can save from the browser. And the interface is a client of the package, so
every limit of the package, single-threaded propagation, no probability-box schedule, no
interval flow, is a limit of the interface as well.
