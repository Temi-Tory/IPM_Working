...
Two questions are asked of a network carrying such a quantity. The first is extremal: what
is the best or worst value that a complete chain of dependencies can produce? The second
concerns room: how far can an individual component move before the extremal answer
changes? The first question is answered by a forward pass. The second is the one that
identifies the critical components, and it can be answered only when the arithmetic of the
quantity can be run backwards.
...
co-authors. Three of those results shape the toolkit.

First, when each duration is an interval, the forward quantities are monotone in the
durations, and their exact bounds are found at the two corners of the input box.

Second, the backward quantities are not monotone. Deciding whether an activity is
possibly critical is NP-complete and computing float bounds is NP-hard
\cite{chanas2002complexity,chanas2003planar,dubois2005floats,fortin2010criticality}.

Third, the forward and backward passes of the classical method are linear algebra over
an idempotent semiring, and the backward pass is a residuation \cite{baccelli1992sync}.
...
reported for a pair without such a backward pass is a number with no meaning.

The modes of Section~\ref{sec:cpm-modes} are built on the third result, and the interval
treatment of Sections~\ref{sec:cpm-interval} and~\ref{sec:cpm-split} on the first two.
...
which every route contributes. Whether the room question has an answer is a property of
the pair, and it has one in two situations.

In the first, $\comb$ is an order ($\max$ or $\min$) and $\prop$ is monotone with a
residual. A bound on the combined value then binds every argument, and the bound inverts
...
In the second, $\comb$ is a sum. A total is not a deadline and no residuation exists.
However, a summed system is linear, and a linear map has an adjoint. The backward object
is therefore a sensitivity, and the room it measures is an allowance against a stated
...
$3$. The same inputs answer opposite questions with opposite critical chains.
...
the total. Under a budget $B$ the room left to node $v$ is
\begin{equation}
...
exact bounds are obtained from two crisp runs of the kernel, one with every input at its
...
\item \emph{Domination split.} For the LongestPath mode with crisp edge values, the exact
...
An earlier implementation of the toolkit ran the scalar kernels on an overloaded interval
type, and it was withdrawn for two reasons. The combination step ranks overlapping
intervals through a partial order that cannot rank them, so the wrong path can be
selected. The margin step subtracts dependent quantities as if they were independent,
which is the dependency problem of Chapter~\ref{ch:lit-review}, so the margins are wider
than the true range. The present toolkit computes each interval quantity by the scheme its
structure permits, and never by running the kernel on an interval type.
...
2 \sum_{v} 2^{|H_v|}
\end{equation}
crisp propagations, one maximising and one minimising sweep per node, against $2^k$ for
...
\begin{algorithm}[htbp]
\caption{Domination split for exact interval floats (LongestPath, crisp edge values)}
\label{alg:cpm-split}
\begin{algorithmic}[1]
\Require graph object, interval durations $[\underline{d}_u, \overline{d}_u]$, crisp edge
  values, run limit
...
The toolkit is checked against three independent computations.

\begin{enumerate}
\item \emph{Path oracle.} A brute-force enumeration of every complete path computes the
  project value, the through-value and the margin of every node for every mode, and the
  critical set, without using the kernels. On the nine networks of the validation set for
  which full enumeration is affordable, from 32 to 306 nodes and from 128 to 404 paths,
  the toolkit agrees with the oracle to within $3 \times 10^{-14}$ in every quantity, and
  the critical sets match. On three networks with between $2.7 \times 10^{6}$ and
  $5.5 \times 10^{18}$ paths the oracle cannot run, and the toolkit is checked instead
  against invariants (no sampled path beats the project value) and, for the accumulation
  mode, against the adjoint identity of Eq.~\eqref{eq:cpm-acc-total}.
\item \emph{Corner enumeration.} Interval margins from the conservative enclosure are
  checked to contain, and those from exhaustive corner enumeration to equal, the margins
  computed by running the path oracle at every corner. The domination split is checked
  to reproduce the exhaustive margins and both criticality sets on every network on which
  both can run.
\item \emph{Monte Carlo sampling.} On networks where exhaustive enumeration is out of
  reach, fifty thousand configurations sampled uniformly from the box are checked never to
  violate the split's bounds, and the bounds are checked to be attained.
\end{enumerate}

The validation scripts and their logs are listed in Appendix~\ref{app:full-results}.

\section{Case Studies}
\label{sec:cpm-cases}

Every number in this section is produced by the toolkit and certified as described in
Section~\ref{sec:cpm-validation}. Timings are wall-clock measurements on a single core of
the second call in a warm process, so that just-in-time compilation is excluded; the
convention is stated in Appendix~\ref{app:full-results}.
...
order the activities differently, which is the information a cost decision needs and which
no scheduling slack expresses.
...
Algorithm~\ref{alg:cpm-order}. The exact result
changes how the deterministic result must be read. Five nodes are necessarily critical,
...
critical. The exact split therefore recovers three necessarily critical activities that
the enclosure cannot see and removes seven activities from the possibly critical set. A
schedule risk assessment that ran only the deterministic pass would name eleven nodes;
one that ran only the enclosure would name 26; the exact interval analysis names 19, and
says which three of them are certain. \PH{Monte Carlo sampling of fifty thousand configurations within the box produced no
violation of the bounds and reached every bound at every node. (MC run still to do.)}
...
selects the exhaustive sweep, and the instance stands at the exhaustive boundary. The
...
The toolkit has the following limits, each a consequence of what has been proved.
...
The split requires crisp edge values. An interval-valued edge reduces to an interval
...
dependence problem as the intervals with no corner argument to rescue them, and no method
for them is claimed.

The cost of the split grows with the bypass sets, and on dense meshes the sets approach the
whole network. The never-worse rule bounds the damage but does not remove it.
...
not check, and the burden of showing it lies with whoever adds the mode. Two extensions
...
been computed exactly from two crisp runs, and the floats exactly by the domination split,
