...
The Critical Path Toolkit implements the schedule and cost interpretation of the network
model. Under this interpretation the value attached to a node is a quantity that accumulates
along dependency paths, such as a duration or a cost, and the value attached to an edge is the amount added or applied in the transfer
...
When $\comb$ is an order ($\max$ or $\min$) and $\prop$ is monotone with a residual, a
margin is defined. A bound on the combined value then binds every argument, and the bound inverts
...
\item \emph{Conservative enclosure.} From the exact bounds on $P$ and $t_v$, the LongestPath
  ...
  \end{equation}
  and correspondingly for the other modes. The enclosure is sound. It costs no more than
  the two corner runs, and it is not exact, because the lower corner of $P$ and the upper
...
\item \emph{Exhaustive corner enumeration.} The margin extremes are attained at corner
  ...
  exact margin range. The cost is $2^k$ runs for $k$ interval inputs. Beyond a stated cap
  the toolkit refuses unless the cap is raised.
...
exact methods tighten. Degenerate intervals, whose two endpoints coincide, are held fixed by every method. They do
not enter the enumeration.
...
exhaustive enumeration of all $k$ interval durations. The exponent now depends on the reconvergent width around each node, and no longer on the
size of the problem. One exhaustive sweep serves
every node at once. The implementation first screens on the number of non-degenerate
interval inputs: below a fixed threshold it computes both counts before running anything
and uses whichever is cheaper, so that within that regime it is never worse than exhaustive
enumeration and refuses only when both counts exceed a stated limit; above the threshold it
declines without pricing the split at all and returns the conservative enclosure. The
screen is a fixed constant and not a function of the network's bypass sets, so a network
with many interval inputs and small bypass sets is declined although the split would have
been cheap. Chapter~\ref{ch:integrated-case-study} meets this case on a real network. Some instances must remain hard,
...
The bypass set is a measure of reconvergence, as the conditioning set of
Chapter~\ref{ch:diamond-module} is, though the toolkit does not use that module. Both
analyses pay for routes that divide and meet again: the decomposition module per join,
through the nodes it must fix, and the split per node, through $H_v$. The two sets index reconvergence at different granularities. Neither contains the other
in general, and neither computation uses the other.
...
18 units, through activities 2, 4, 6, 10, 25 and 30. The two modes share activity 30 and
...
total 362). Allowance is headroom divided by path multiplicity. The first eight rows are
the largest contributors; node 6 is appended below the rule because the text discusses its
allowance despite a smaller contribution.}
...
\begin{figure}[htbp]
  ...
  20 per cent. Black bars are the possibly critical nodes, whose lower bounds reach zero.
  Grey bars are the activities whose floats stay positive everywhere in the box. The
  necessarily critical nodes are drawn as points at zero, since both bounds of their float
  are zero.}
...
whole network. The never-worse rule keeps the cost at or below that of exhaustive
enumeration, and no lower. Separately, the screening threshold that decides whether the
split is attempted is a fixed count of interval inputs, chosen for safety and not derived
from the network's bypass-set cost; computing that cost first, which is cheap, would let
more networks reach the exact method.
...
The classical critical path method has been generalised to a family of analysis modes, each an operator pair with the
