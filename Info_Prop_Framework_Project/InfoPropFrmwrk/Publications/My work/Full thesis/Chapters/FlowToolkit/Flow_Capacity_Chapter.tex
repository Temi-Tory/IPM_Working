...
The base flow solver run returns $F^\star = 37.0$, with $\texttt{mincut\_capacity} = 37.0$ for both networks, which is the maximum feasible flow for the system as built. However, $F^\star$ alone does not distinguish the two designs. As shown in Table~\ref{tab:config_comparison}, the two networks still differ in structure: Configuration A has 41 saturated edges versus 35 in Configuration B, and 4 valid minimum cuts versus 1. This is also reflected in free-zone size (2 in A, 0 in B). The two designs have the same throughput under different constraints, which is what the remaining modules distinguish.
...
\begin{figure}[htbp]
  ...
  nine-line binding cut of the structural-subset model, which is a transmission bottleneck
  only when the mixed buses are excluded from supply and demand.}
...
Under this model the network delivers the full 1607~MW to the import buses, so that all
2850~MW of peak demand is met. The result is the same with generator capacities applied and
with generation unconstrained, so at peak load neither the lines nor the generators bind:
the maximum flow is limited by the demand itself, and the binding cut is the set of eleven
edges into the super-sink. No bus is a single point of failure. The single-line and
single-generator failure rankings are both empty, because every line and every generator has
slack in the flow that delivers 1607~MW, so no single failure reduces the aggregate
deliverable throughput. This is a statement about aggregate throughput under a flow model and
not about operational security, since a remaining path could overload under a real
redispatch, which the model does not represent. The minimum cut is unique: the lattice of Section~\ref{sec:flow-mincut} has an empty free
zone and the enumeration is complete, and no generator edge lies in it. The nearest
constraints are the four generators at buses 1, 2, 13 and 16, which are at their nameplate
limit in the maximum flow found, and line 7--8, which carries 171 of its 175~MVA. These are the assets at which the system would bind first
if demand grew (Table~\ref{tab:flow-rts24}).
...
the structural-subset model measures a transmission bottleneck between buses that do not
exist as pure sources and sinks in the real system.
