...
A network that is connected can still fail to deliver. When a pipe is throttled or a feeder
is derated, supply reaches every demand point and the amount that arrives is short of what
is needed. The probabilistic interpretation of Chapter~\ref{ch:probability-toolkit} asks
whether supply reaches a node;
...
\paragraph{Structural single points of failure.}
...
Real failure events are not always isolated to a single component. A storm in a power network can take down multiple edges simultaneously. Since the interaction between affected edges may open or close re-routing options, the combined impact of $k$ simultaneous failures is not in general the sum of their individual $\Delta_{uv}$ impacts.
