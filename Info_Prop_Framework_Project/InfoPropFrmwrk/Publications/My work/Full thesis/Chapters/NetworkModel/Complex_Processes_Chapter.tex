...
acyclicity assumption. Section~\ref{sec:model-roles} gives the physical reading of the node
roles, and Section~\ref{sec:model-formal} states the definitions and notation used throughout
the thesis. Section~\ref{sec:model-layers} establishes the layered processing order and the
...
network, the three readings of those values, and the three forms in which a value can be
...
The model further requires that the directed edges form no cycle. For a large class of the
systems of interest this is simply true. For example, treatment trains, supply chains, assembly
processes and project schedules move material or work strictly forward, and a dependency cycle
in a schedule would be a planning error. For other systems, however, acyclicity is an
orientation of a richer reality.
...
dominant flow gives a directed acyclic snapshot of the operating regime. What the orientation
gives up is the analysis of regimes far from the snapshot, such as flow reversal under unusual
demand, recirculation loops and feedback control. The model excludes these by design, and
...
The position of a node in the network gives it a role, and each role has a physical reading. A
node with no incoming edge is a source, the point where supply, material or work enters the
system (e.g.\ an intake, a generator or the start of a project). Similarly, a node with no
outgoing edge is a sink, where the system delivers (e.g.\ a demand point, an outlet or a
completed handover). A node that feeds two or more downstream components is a fork. Forks are
...
two or more components is a join. This is where distributed flows gather, where assembled parts
meet, and where redundant routes come back together.

The last of these readings is a fact about engineered systems as well as about graphs. A
robust system duplicates its critical routes by design, and duplicated routes must
reconverge at the destination they protect. A well-designed network therefore contains many
...
Chapter~\ref{ch:probability-toolkit} shows that for the probabilistic reading of the model,
ignoring it gives wrong answers. For the present chapter it is enough to note that
reconvergence is a designed feature of the systems being modelled. Figure~\ref{fig:model-roles}
...
Every analysis in the framework visits the nodes of the network in an order in which each
node is processed after all of its parents. For a DAG such an order always exists. A
topological sort of $G$ is a linear ordering of $V$ in which $u$ precedes $v$ for every edge
...
The theorem matters for the framework in two ways. First, every analysis in the following
chapters is an exact method with no iterative convergence loop. Hence a fixed processing order
declared once serves all of them, and the number of steps is known from the structure before
any analysis runs. Second, nodes within a layer share no dependency, so a layer is a natural
boundary for parallel processing by any algorithm that supports it. The layers are computed by a
...
contains every other node, giving $O(|V|^2)$ space. In return, any reachability query reduces
to a single set membership test in constant time. This is a trade of space for time, justified
by the number of such queries the decomposition and propagation algorithms make.
...
The model separates what the network is from what is asked of it. The topology of
Definition~\ref{def:model-network} is fixed once per system, and an analysis attaches values to
its nodes and edges whose meaning depends on the question. Under the probabilistic reading a
...
the capacity reading the values are throughput limits, and the question is how much the network
...
reading the values are durations or costs, of the component's own processing and of the
...
Critical Path Toolkit of Chapter~\ref{ch:critical-path}. The three readings are three
...
in which real systems arrive, which is the subject of the next section, and for the
probabilistic reading the reconvergent structure of Section~\ref{sec:model-roles} must be
identified, which is the subject of Chapter~\ref{ch:diamond-module}. Both are preprocessing
steps performed once per system.
...
Different objectives are pursued on the same system, a fault-detection workflow tracks health
signals while a planning workflow tracks costs, and each maintains the parameters relevant to
its own purpose. Furthermore, the tools used across these workflows implement different schemas and export
...
nodes are point features such as sources, junctions, assets or demand points and edges are the
...
model, parses each from its own file, and produces the graph object every analysis reads. It is
the single mandatory entry point of the framework: no analysis can be invoked without it.
...
it scans the content for the integer, delimiter, integer pattern of an edge record; failing
that, it treats a two-column table as an edge list and anything else as an adjacency matrix.
...
is needed to construct the reverse adjacency. Ingestion is therefore linear in the number of
edges, and the parser returns the minimal graph: the edge list, the two indices and the
sources.
...
structure, in manufacturer specifications, historic records or planning estimates, and the
...
It would also force a user who wants to evaluate the same network under several readings, or
under several scenarios of one reading, to keep a duplicate topology file for each.

All analysis inputs are given as JSON files. JSON is human-readable, platform-independent, and
maps naturally onto the key-value form in which component attributes are described. Where a
flat format such as CSV suits the rigid tabular shape of an edge list, JSON's nesting suits the
values themselves. An interval or a p-box can be written directly as a nested object without
inventing column conventions or flattening a structured value into several cells. No specialist
tool is needed to write or edit such a file, and a data owner who understands the node and edge
identifiers can contribute values without knowledge of graph theory.
...
optional list of node capacities. Since capacity is point-valued in the present framework,
...
object per invocation, so that one instance of a network can be evaluated under different
readings and under different scenarios of one reading without rebuilding the structure. Every
argument that any analysis in the framework requires can be taken from this object, and it is
...
\begin{enumerate}
  ...
  \item Under the probabilistic reading a component is binary, operational or failed, and
...
Every analysis in the following chapters assumes this list, and none of the algorithms
introduces an assumption beyond it. Where a later chapter relaxes or adds to the list for a
particular reading, it says so.
...
readings. The graph object built from the model carries the topology, a layered processing
...
object under one of three readings, probabilistic, capacity, or schedule and cost, in one of
...
domains at parse time. The four assumptions of Section~\ref{sec:model-assumptions} apply to
every later result.
