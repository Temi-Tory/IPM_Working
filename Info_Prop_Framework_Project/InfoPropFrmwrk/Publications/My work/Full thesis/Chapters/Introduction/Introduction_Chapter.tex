...
other. A water treatment works, for instance, abstracts raw water from a river,
...
framework developed here the three analyses are therefore independent interpretations of one network
...
DAG model of a system on which reliability, capacity and schedule analysis are computed, and it
propagates interval and p-box inputs through each analysis as far as the mathematics of that
analysis allows. It is provided as a Julia package together with a no-code user interface for
...
\begin{figure}[htbp]
  ...
  \caption{The layers of the Information Propagation Framework, with the value forms each
    analysis toolkit supports.}
...
Several engineering software tools already run more than one kind of analysis against one
shared network object, as reviewed in Chapter~\ref{ch:lit-review}. No tool or method found in
that review combines source-to-node reliability, capacitated flow and activity scheduling on
one network representation. The closest work pairs two of the three. Huang, Huang
...
\begin{enumerate}
  ...
  \item Can exact source-to-node reliability be computed when component probabilities are
    ...
    does the computation cost relative to decision-diagram methods?
...
\begin{itemize}
  \item To define a network model general enough that reliability, capacity and schedule
    analysis are each an interpretation of the same object, and to build that object from the file
    formats in which infrastructure data commonly arrives.
  \item To develop exact analysis methods for each of the three interpretations, and to extend each to
    interval and p-box inputs as far as the mathematics of that interpretation allows.
...
\begin{enumerate}
  ...
  \item \textbf{The Critical Path Toolkit, generalised over the propagated quantity.} The
    ...
    of the input box. The floats are computed exactly by a domination split that reduces
...
analysis and the exact methods for network reliability, flow and scheduling. It then examines
the tools and methods that combine more than one of these analyses on one network, and the
patterns in which engineering analysis software is delivered to its users.
...
sources, sinks, forks and joins are given their physical meaning. The chapter then establishes
...
scope of the toolkit, demonstrates it on a synthetic worked example under two capacity
configurations, and applies it to the 24-bus IEEE Reliability Test System as a real-network case
study. Timings on generated instances of a DIMACS mesh family are also reported.
...
three interpretations of one set of inputs, and the exact interval floats are compared with the
...
types of the graph object, the value helpers over which the propagation is generic, and the
server. It also records why a purpose-built graph object was preferred to the language's
general graph library, and how the package is delivered through the Julia General registry.
...
\begin{itemize}
  ...
  \item Ohiani, T., Patelli, E. and Pyke, C. ``Information Propagation Algorithm for Exact
    Reliability Analysis in Directed Acyclic Process Networks,'' submitted to
    \textit{Reliability Engineering \& System Safety} (under revision). This paper covers
    the exactness proofs and the interval and probability-box extensions of
    Chapter~\ref{ch:probability-toolkit}.
  \item Ohiani, T., Patelli, E. and Pyke, C. Conference paper on the no-code local interface, (in preparation for ESREL World 2027).
  \item Ohiani, T., Patelli, E. and Pyke, C. Conference paper on the Julia package, (in preparation).
\end{itemize}
