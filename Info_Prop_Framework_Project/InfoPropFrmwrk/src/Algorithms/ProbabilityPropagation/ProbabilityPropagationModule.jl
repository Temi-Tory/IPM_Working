...
# Mathematical Foundation (see ProbabilityPropagationModule_Maths.md for details)
...
4. **Optimizations** (over V1_ProbabilityPropagationModule):
...
Arithmetic uses InputProcessingModule helpers (multiply_values, add_values, etc.)
so that pbox/Interval arithmetic is handled transparently.
"""
module ProbabilityPropagationModule
