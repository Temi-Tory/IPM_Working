...
function pbox_update_beliefs_iterative(
    ...
    # Use the minimal validation that skips problematic checks
    ...
    for node_set in iteration_sets
        for node in node_set
            ...
            if length(all_beliefs) == 1
                ...
                belief_dict[node] = pbox_inclusion_exclusion(all_beliefs)
            end
        end
    end
...
# Helper function to combine p-boxes with independence assumption
...
function pbox_inclusion_exclusion(belief_values::Vector{pbox})
    ...
    for i in 1:num_beliefs
        ...
        for combination in combinations(1:num_beliefs, i)
            # Get the p-boxes for this combination
            combo_pboxes = [belief_values[j] for j in combination]
            
            # Calculate the intersection probability of the current combination
            # For independent events, intersection is product
            intersection_probability = pbox_combine_pboxes(combo_pboxes, :*)
...
function pbox_updateDiamondJoin(
    ...
    # Success case (fork = 1)
    sub_node_priors[fork_node] = PBA.makepbox(PBA.interval(1, 1))
    success_belief = pbox_update_beliefs_iterative(
        subgraph.edgelist,          
    ...
    # Failure case (fork = 0)
    sub_node_priors[fork_node] = PBA.makepbox(PBA.interval(0, 0))
    failure_belief = pbox_update_beliefs_iterative(
        subgraph.edgelist,          
        sub_iteration_sets,  # Use fresh iteration sets
        sub_outgoing_index,  # Use fresh outgoing index
        sub_incoming_index,  # Use fresh incoming index
    ...
    # (success_belief * original_fork_belief) + (failure_belief * (1 - original_fork_belief))
    success_term = PBA.convIndep(success_belief, original_fork_belief, op = *)
    
    # Calculate (1 - original_fork_belief) using p-boxes
    one_pbox = PBA.makepbox(PBA.interval(1, 1))
    complement_fork = PBA.convIndep(one_pbox, original_fork_belief, op = -)
    
    failure_term = PBA.convIndep(failure_belief, complement_fork, op = *)
    updated_belief_dict[join_node] = PBA.convIndep(success_term, failure_term, op = +)
