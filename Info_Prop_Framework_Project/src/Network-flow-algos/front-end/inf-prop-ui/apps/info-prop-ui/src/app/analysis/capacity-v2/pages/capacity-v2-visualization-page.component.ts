...
@Component({
  selector: 'app-capacity-v2-visualization-page',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  templateUrl: './capacity-v2-visualization-page.component.html',
  styleUrl: './capacity-v2-visualization-page.component.scss'
})
export class CapacityV2VisualizationPageComponent {
  readonly store = inject(CapacityV2Store);

  // Full screen dimensions
  readonly width = computed(() => {
    if (typeof window !== 'undefined') {
      return Math.max(window.innerWidth - 300, 1200); // Account for sidenav
    }
    return 1200;
  });
  ...
  readonly height = computed(() => {
    if (typeof window !== 'undefined') {
      return window.innerHeight - 200; // Account for toolbar
    }
    return 800;
  });
  ...
  readonly nodePositions = computed(() => {
    ...
    const xStep = w / Math.max(layers.length, 1);

    layers.forEach((layerNodes, layerIndex) => {
      layerNodes.forEach((nodeId, nodeIndex) => {
        const yStep = h / (Math.max(layerNodes.length, maxLayerSize) + 1);
  ...
  onHighlightModeChange(event: MatSelectChange): void {
    this.store.setHighlightMode(event.value as CapacityV2HighlightMode);
  }

  zoomIn(): void {
    this.zoomSignal.set(Math.min(this.zoomSignal() + 0.1, 3));
  }

  zoomOut(): void {
    this.zoomSignal.set(Math.max(this.zoomSignal() - 0.1, 0.3));
  }
