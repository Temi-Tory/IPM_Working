        ...
        for _ in 1:level_size
            ...
            if node in join_nodes
                if haskey(incoming_index, node)
                    ...
                    if length(parents) > 1
                        common_ancestors = pre_process_diamond_subgraph(parents, ancestors, source_nodes, fork_nodes)
...
edgelist, outgoing_index, incoming_index, source_nodes = read_graph_to_dict("csvfiles/KarlNetwork.csv")
fork_nodes, join_nodes = identify_fork_and_join_nodes(outgoing_index, incoming_index)
iteration_sets, ancestors, descendants, common_ancestors_dict = find_iteration_sets(edgelist, outgoing_index, incoming_index, fork_nodes, join_nodes, source_nodes)
...
for (join_node, common_ancestors) in common_ancestors_dict
    ...
    if length(common_ancestors) > 1
        println("Processing nested diamond subgraphs:")
        nested_diamonds = post_process_diamond_subgraph(decomposed_edges, join_node, common_ancestors)
        for (nested_join_node, nested_edges) in nested_diamonds
            println("  Nested Join Node: ", nested_join_node)
            println("  Nested Decomposed Edgelist:")
